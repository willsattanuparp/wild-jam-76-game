...
func attack_consecutive(type: int,number_of_projectiles: int,direction: Vector2, time_between_projectiles: float, direct_at_player: bool, time_between_attacks: float, number_of_attacks: int, point: Vector2 = Vector2.ZERO):
	...
	for i in number_of_attacks:
		match type:
			0:
				animation_player.play("deer_throw")
			1:
...
func play_jump_animation(anim_speed):
	...
	animation_player.play("deer_jump")

func movement_animations(anim_speed):
	if !jumping:
		if direction.x != 0 and !(animation_player.is_playing() and (animation_player.current_animation == "deer_walk" or animation_player.current_animation == "deer_throw")):
			animation_player.speed_scale = anim_speed
			animation_player.play("deer_walk")
		elif direction.x == 0 and (animation_player.is_playing() and (animation_player.current_animation == "deer_walk" or animation_player.current_animation == "deer_throw")):
			animation_player.stop()
		if animation_player.is_playing() and animation_player.current_animation == "deer_walk":
			if velocity.x > 0:
				animation_player.speed_scale = -anim_speed
			elif velocity.x < 0:
				animation_player.speed_scale = anim_speed
